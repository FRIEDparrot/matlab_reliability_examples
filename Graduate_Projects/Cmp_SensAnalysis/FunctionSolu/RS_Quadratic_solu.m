%% %%%%%%%%%%%%%%%%%%%%%% 加权二阶响应面模型 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Pf, model] = RS_Quadratic_solu(mu_, sigma_, g, mode)
% model: the predicted result is b1 * 1 + b2 * x(1) + b3 * x(2).. + bn+1 x(1)^2 + ...
%        use model.g for predict results 
% mode: 1:use WRSM weight function     2: use Joint density funciton for weight function
    n =  size(mu_,2);
    ...
    elseif mode == 2
        fx = @(xp) NPfunc(xp,mu_,sigma_d);                % 计算联合概率密度函数, 查一下源码
        weight_func = @(gx,xp) min((abs(gx) + 1e-5)./fx(xp)) ./ ((abs(gx) + 1e-5)./fx(xp));
    else
        error("mode = 1: for ")
    end
